...
import argparse
import codecs
from rsgt import config
from rsgt.face import get_face_boxes, get_face_landmarks, facedata
from rsgt.eye import eyedata
#from rsgt.iris_detectors.ert_detector import ert_detector as iris_detector
from rsgt.iris_detectors.enet_detector import enet_detector as iris_detector
from rsgt.util import calc_gaze_position, get_eye_rotation
from rsgt.screen import screen
...
if __name__ == '__main__':
    ...
    for f in files:
        ...
        if len(dets) > 0: # face is found
            ...
            line = '{},'.format(f)
            line += '{},'.format(scores[0])
            line += '{},{},{},'.format(*(180*face.euler_angles/np.pi))
            line += '{},{},{},'.format(*np.ravel(face.translation_vector))
            ...
            if not left_eye.blink:
                line += '{},{},'.format(*left_eye.normalize_coord(left_eye.iris_center))
                #line += '{},{},'.format(*get_eye_rotation(face, left_eye))
            else:
                line += ',,'
            if not right_eye.blink:
                line += '{},{},'.format(*right_eye.normalize_coord(right_eye.iris_center))
            ...
            # screen position
            if not left_eye.blink:
                line += '{},{},'.format(*calc_gaze_position(face, left_eye, dummy_screen, None, None))
            else:
                line += ',,'
            if not right_eye.blink:
                line += '{},{}'.format(*calc_gaze_position(face, right_eye, dummy_screen, None, None))
            else:
                line += ','
